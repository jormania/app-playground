...
5. **Lite Practice (Settings toggle)**:
   ...
   - **Settings narrows too**: the Socratic Mentor section (the app's only AI surface — `MentorPanel` is mounted in the stepper, Commitments and the Council, none of which Lite renders) is hidden, and Habit Reminders drops to an evening-only nudge. Settings writes `morningEnabled: false` into the reminders IndexedDB state and [`daily-stoic-sw.js`](public/daily-stoic-sw.js) skips the morning notification on an explicit `false` — a missing key still fires it, so already-installed workers are unaffected.
