...
- **Context-Aware Cleanups**: Uses a deterministic cleaning pipeline that preserves conversational prepositions inside the description (e.g. "Dinner with friends") while cleanly stripping structural prepositions at the ends of phrases. It derives natural-sounding descriptions like "Uber to school" instead of rigid, robotic extractions.
- **Explicit Category Mapping**: The inference engine perfectly maps unknown keywords to the exact user categories via heuristics (e.g., "netflix" -> `Leisure`, "uber" -> `Transport`), while gracefully falling back to a blank category if the input is entirely unmatched.
- **Smart Default Accounts**: When no specific account is requested by name (e.g., "15 for Uber"), it intelligently falls back to the plain `Revolut` account (or the base currency account) by prioritizing exact string matches, rather than incorrectly picking up foreign currency accounts like `Revolut (EUR)`.
