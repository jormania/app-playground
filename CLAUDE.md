...
| KeyPath | `src/keypath/` | **strict TS**, DS — **a hardware probe, not an app yet** — passed on the S24 (2026-09-24, see KEYPATH.md §1); Nora's Poco F3 still to check. Unlisted (no registry entry, no service worker). The MIDI layer in `midi/` must stay free of React. **No music files in the repo, ever** — `content-boundary.test.js` enforces it. Read [`KEYPATH.md`](KEYPATH.md) |
