...
| Marquee | `src/marquee/` | JSX, DS — venue watcher; reads venue programme pages through `api/marquee-scan.js` (the ONE endpoint, adapters under `api/_lib/marquee/`), saves into Wanderlist's Findings. Read [`MARQUEE.md`](MARQUEE.md) — especially the health gate, "throttled ≠ broken", and the detail cache (§9.75, why an adapter's `extractDetail` exists) — before touching an adapter |
