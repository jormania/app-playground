...
Everything MUST fit the real time of day stated below. At night never invoke the sun, daylight, dawn, or "chasing the light"; by day never invoke the moon, stars, or the dark. Match what is actually outside right now.`,
  ...
  return {
    ...
    reading: reading || (mode === 'tonight' ? randomTonight() : null),
  }
}
...
function clampTonight(s) {
  ...
  return randomTonight()
}
// a complete Tonight line, for when the oracle is silent (no key, an error, or a
// dropped field) — so the reading is never the bare label "Tonight" alone
function randomTonight() {
...
export default function DeparturePanel({ onDepart, apiKey, fill = 'almanac' }) {
  ...
  useEffect(() => {
    if (!apiKey) {
      ...
      if (fill === 'tonight') setReading(randomTonight())
      setLoading(false)
      return
    }
