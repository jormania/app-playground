...
export function getDayRecord(dictionary: string, date: string = today()): DayRecord {
 ...
 * the same way they count against lifetime stats.
 */
export function recordFinishedGame(dictionary: string, won: boolean, date: string = today()): DayRecord {
  const store = readJson<Store>(KEY, {})
