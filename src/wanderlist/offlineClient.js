// Makes Wanderlist usable with no connection. Wraps the live Notion client so the UI
// keeps the same listEntries / createEntry / updateEntry interface, but:
//   • the last successful read is CACHED locally, so the backlog is browsable offline;
//   • writes made offline go into an OUTBOX and sync to Notion on reconnect.
// Fixtures mode is already fully local, so only live mode is wrapped. Ported from JoD's
// offlineClient (minus the photo passthroughs — no attachments in M1).
//
// Two localStorage stores, keyed by databaseId so pointing the app at a different
// database can never cross-contaminate cached items or queued writes.
const CACHE_PREFIX = 'wanderlist_cache:'
const OUTBOX_PREFIX = 'wanderlist_outbox:'

// Unique even for two writes in the same millisecond.
let seq = 0
...
// A failed save is "offline" (queue it) when the network never answered — a true fetch
// rejection is a TypeError. A proxy Error WITH a message (bad token, DB not shared, 4xx)
// is a real error we must surface, not swallow.
...
// Lay queued writes over a base list, tagging each touched item `pending: true` so the
// UI can mark it "not yet synced". Creates surface under a temp id.
function applyOutbox(base, outbox) {
  const list = (base || []).map(e => ({ ...e }))
  for (const op of outbox) {
    const optimistic = { ...op.entry, pending: true }
    if (op.kind === 'create') {
...
export function createOfflineClient(inner, { databaseId = 'default' } = {}) {
  ...
  const removeOp = (id) => setOutbox(getOutbox().filter(o => o.tempId !== id))
  ...
  return {
    ...
    async createEntry(entry) {
      ...
      return { ...entry, id, pending: true }
    },

    async updateEntry(id, entry) {
      // Editing an item still queued (a pending create or a prior pending edit): amend
      // the queued op in place rather than stacking a second op against a temp id Notion
      // will never know.
      const outbox = getOutbox()
