...
describe('buildReport', () => {
  it('leads with what went right, and gives three stars for a clean piece', () => {
    const r = buildReport(runPerfect(DEFAULT_SETTINGS), DEFAULT_SETTINGS)
  ...
  it('gives no stars for an abandoned piece', () => {
    const j = new Judge(odeToJoy, { practice: 'both', settings: DEFAULT_SETTINGS })
    j.start(0)
    j.press(64, 0)
    expect(buildReport(j.summary(), DEFAULT_SETTINGS).stars).toBe(0)
  })
  ...
  it('names one bar to work on in a short report, every troubled bar in a detailed one, none when off', () => {
    ...
    const j = new Judge(song, { practice: 'both', settings: DEFAULT_SETTINGS })
    j.start(0)
    j.press(60, 0)
    j.press(62, 500) // bar 0 clean
    j.press(70, 2100) // bar 1: wrong key…
    j.tick(9999) // …and both its notes missed; bar 2 missed too
    const short = buildReport(j.summary(), DEFAULT_SETTINGS)
    expect(short.toWorkOn.map((b) => b.bar)).toEqual([1])
    const detailed = buildReport(j.summary(), { ...DEFAULT_SETTINGS, report: 'detailed' })
    expect(detailed.toWorkOn.map((b) => [b.bar, b.missed, b.wrong])).toEqual([
      [1, 2, 1],
      [2, 1, 0],
    ])
    expect(buildReport(j.summary(), { ...DEFAULT_SETTINGS, report: 'off' }).toWorkOn).toEqual([])
    expect(short.highlights).toContainEqual({ kind: 'cleanBars', bars: [0] })
  })

  it('suggests one step up after a piece that went very well — the next wrong-note mode first, then timing', () => {
    expect(buildReport(runPerfect({ ...DEFAULT_SETTINGS, onWrong: 'keepGoing' }), { ...DEFAULT_SETTINGS, onWrong: 'keepGoing' }).suggestion).toEqual({ setting: 'onWrong', to: 'show' })
    expect(buildReport(runPerfect(DEFAULT_SETTINGS), DEFAULT_SETTINGS).suggestion).toEqual({ setting: 'onWrong', to: 'wait' })
    const strictest = { ...DEFAULT_SETTINGS, onWrong: 'wait' as const, timing: 'strict' as const }
    expect(buildReport(runPerfect(strictest), strictest).suggestion).toBeNull()
  })
  ...
  it('does not suggest anything after a piece that went only fairly well', () => {
    ...
    j.tick(99_999)
    const r = buildReport(j.summary(), DEFAULT_SETTINGS)
