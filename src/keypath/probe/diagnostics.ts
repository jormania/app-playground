...
export function evaluate(kind: TestKind, events: readonly NoteEvent[], finished = false): TestResult {
  ...
  switch (kind) {
    ...
    case 'chord': {
      if ((p.ons.length < CHORD.length || anyHeld) && !finished) {
        return { verdict: 'waiting', summary: p.ons.length ? `Holding ${p.ons.length} note(s)…` : 'Waiting for the chord…', details: [] }
      }
      const got = new Set(p.ons.map((e) => e.note))
      ...
      const chordOns = p.ons.filter((e) => CHORD.includes(e.note))
      const times = chordOns.map((e) => e.time)
      ...
      return verdict(problems, `All three chord notes received as separate Note Ons.`, [
        `Arrival order: ${chordOns.map((e) => noteName(e.note)).join(' → ')}`,
        `Onset spread (first to last key of the chord): ${ms(spread)}`,
        `Velocities: ${chordOns.map((e) => `${noteName(e.note)} ${e.velocity}`).join(', ')}`,
